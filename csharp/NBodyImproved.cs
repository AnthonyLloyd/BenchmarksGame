...
public static class NBodyImproved
{
    ...
    static Body[] createBodies()
    {
        ...
        var jupiter = new Body {
                ...
                vx = 1.66007664274403694e-03 * dt * DaysPeryear,
                vy = 7.69901118419740425e-03 * dt * DaysPeryear,
                vz = -6.90460016972063023e-05 * dt * DaysPeryear,
                mass = 9.54791938424326609e-04 * dt * dt * Solarmass,
            };
        var saturn = new Body {
                x = 8.34336671824457987e+00,
                y = 4.12479856412430479e+00,
                z = -4.03523417114321381e-01,
                vx = -2.76742510726862411e-03 * dt * DaysPeryear,
                vy = 4.99852801234917238e-03 * dt * DaysPeryear,
                vz = 2.30417297573763929e-05 * dt * DaysPeryear,
                mass = 2.85885980666130812e-04 * dt * dt * Solarmass,
            };
        var uranus = new Body {
                x = 1.28943695621391310e+01,
                y = -1.51111514016986312e+01,
                z = -2.23307578892655734e-01,
                vx = 2.96460137564761618e-03 * dt * DaysPeryear,
                vy = 2.37847173959480950e-03 * dt * DaysPeryear,
                vz = -2.96589568540237556e-05 * dt * DaysPeryear,
                mass = 4.36624404335156298e-05 * dt * dt * Solarmass,
            };
        var neptune = new Body {
                x = 1.53796971148509165e+01,
                y = -2.59193146099879641e+01,
                z = 1.79258772950371181e-01,
                vx = 2.68067772490389322e-03 * dt * DaysPeryear,
                vy = 1.62824170038242295e-03 * dt * DaysPeryear,
                vz = -9.51592254519715870e-05 * dt * DaysPeryear,
                mass = 5.15138902046611451e-05 * dt * dt * Solarmass,
            };
        var sun = new Body {
                mass = dt * dt * Solarmass,
                vx = (jupiter.vx * jupiter.mass + saturn.vx * saturn.mass
                        +uranus.vx * uranus.mass + neptune.vx * neptune.mass)/(-dt * dt * Solarmass),
                vy = (jupiter.vy * jupiter.mass + saturn.vy * saturn.mass
                        +uranus.vy * uranus.mass + neptune.vy * neptune.mass)/(-dt * dt * Solarmass),
                vz = (jupiter.vz * jupiter.mass + saturn.vz * saturn.mass
                        +uranus.vz * uranus.mass + neptune.vz * neptune.mass)/(-dt * dt * Solarmass),
            };
    ...
    public static void Main(String[] args)
    {
        ...
        Console.WriteLine(startEnergy.ToString("f9"));
    ...
    public static Tuple<double,double> Test(String[] args)
    {
        ...
        for(int n = args.Length > 0 ? Int32.Parse(args[0]) : 10000; n>0; n--)
            advance(bodies, pairs);
        return Tuple.Create(Math.Round(startEnergy,10), Math.Round(energy(bodies, pairs),10));
    }
}