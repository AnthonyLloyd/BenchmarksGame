...
let inline sunMomentum planets =
...
let energy {bodies=bodies;pairs=pairs} =
    let ePlanets = bodies |> List.sumBy (fun b ->
        0.5 * b.Mass * (b.VX*b.VX + b.VY*b.VY + b.VZ*b.VZ))
    let ePairs = pairs |> List.sumBy (fun (b1,b2) ->
        let dx,dy,dz = b1.X-b2.X, b1.Y-b2.Y, b1.Z-b2.Z
        b1.Mass * b2.Mass / sqrt (dx*dx + dy*dy + dz*dz))
    ePlanets - ePairs
